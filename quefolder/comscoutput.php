...
<?php                               
                                ...
                                $fetchqry2 = "UPDATE `comscoutput` SET `userans`='$userselected' WHERE `id`=$c-1"; 
...
<table>
  <?php if(isset($c)) {   $fetchqry = "SELECT * FROM `comscoutput` where id='$c'"; 
...
<div class="option">
<tr><td><h1><br><?php echo @$row['que'];?></h1></td></tr> <?php if($_SESSION['clicks'] > 0 && $_SESSION['clicks'] < 4){ ?>
...
<?php }  
 ...
 <?php if($_SESSION['clicks']>3){ 
  $qry3 = "SELECT `ans`, `userans` FROM `comscoutput`;";
...
?>
<div class="resulted">
<h1>The Answers are</h1>
 <?php
$sql = "SELECT id,ans FROM comscoutput";
