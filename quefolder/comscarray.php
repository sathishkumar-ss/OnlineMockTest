...
<?php                               
                                ...
                                $fetchqry2 = "UPDATE `comscarray` SET `userans`='$userselected' WHERE `id`=$c-1"; 
...
<table>
  <?php if(isset($c)) {   $fetchqry = "SELECT * FROM `comscarray` where id='$c'"; 
...
<div class="option">
<tr><td><h1><br><?php echo @$row['que'];?></h1></td></tr> <?php if($_SESSION['clicks'] > 0 && $_SESSION['clicks'] < 14){ ?>
...
<?php }  
 ...
 <?php if($_SESSION['clicks']>13){ 
  $qry3 = "SELECT `ans`, `userans` FROM `comscarray`;";
...
<h3>The Answers are</h3>
  <?php
$sql = "SELECT id,ans FROM comscarray";
...
<a href="cs.php" style="font-size: 24px;text-decoration: none;">GoHome</a>
 </div>
<?php } ?>
</div>
 </body>
