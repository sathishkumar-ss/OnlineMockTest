...
<?php                               
                                ...
                                $fetchqry2 = "UPDATE `airwaysgrammar` SET `userans`='$userselected' WHERE `id`=$c-1"; 
...
<table>
  <?php if(isset($c)) {   $fetchqry = "SELECT * FROM `airwaysgrammar` where id='$c'"; 
...
<tr><td><h1><br><?php echo @$row['que'];?></h1></td></tr> <?php if($_SESSION['clicks'] > 0 && $_SESSION['clicks'] < 14){ ?>
 ...
 <?php if($_SESSION['clicks']>13){ 
  $qry3 = "SELECT `ans`, `userans` FROM `airwaysgrammar`;";
 ...
 <h3>The Answers are</h3>
  <?php
$sql = "SELECT id,ans FROM airwaysgrammar";
