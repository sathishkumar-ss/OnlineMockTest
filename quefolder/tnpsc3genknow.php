...
<?php                               
                                ...
                                $fetchqry2 = "UPDATE `tnpsc3genknow` SET `userans`='$userselected' WHERE `id`=$c-1"; 
...
<table>
  <?php if(isset($c)) {   $fetchqry = "SELECT * FROM `tnpsc3genknow` where id='$c'"; 
...
<tr><td><h1><br><?php echo @$row['que'];?></h1></td></tr> <?php if($_SESSION['clicks'] > 0 && $_SESSION['clicks'] < 10){ ?>
 ...
 <?php if($_SESSION['clicks']>9){ 
  $qry3 = "SELECT `ans`, `userans` FROM `tnpsc3genknow`;";
 ...
 <h3>The Answers are</h3>
  <?php
$sql = "SELECT id,ans FROM tnpsc3genknow";
