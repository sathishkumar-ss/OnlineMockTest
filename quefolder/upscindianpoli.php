...
<?php                               
                                ...
                                $fetchqry2 = "UPDATE `upscindianpoli` SET `userans`='$userselected' WHERE `id`=$c-1"; 
...
<table>
  <?php if(isset($c)) {   $fetchqry = "SELECT * FROM `upscindianpoli` where id='$c'"; 
...
<div class="option">
<tr><td><h1><br><?php echo @$row['que'];?></h1></td></tr> <?php if($_SESSION['clicks'] > 0 && $_SESSION['clicks'] < 9){ ?>
...
<?php }  
 ...
 <?php if($_SESSION['clicks']>8){ 
  $qry3 = "SELECT `ans`, `userans` FROM `upscindianpoli`;";
...
<h3>The Answers are</h3>
  <?php
$sql = "SELECT id,ans FROM upscindianpoli";
