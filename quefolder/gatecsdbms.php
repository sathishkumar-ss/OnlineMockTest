...
<?php                               
                                ...
                                $fetchqry2 = "UPDATE `gatecsdbms` SET `userans`='$userselected' WHERE `id`=$c-1"; 
...
<table>
  <?php if(isset($c)) {   $fetchqry = "SELECT * FROM `gatecsdbms` where id='$c'"; 
...
<tr><td><h1><br><?php echo @$row['que'];?></h1></td></tr> <?php if($_SESSION['clicks'] > 0 && $_SESSION['clicks'] < 8){ ?>
 ...
 <?php if($_SESSION['clicks']>10){ 
  $qry3 = "SELECT `ans`, `userans` FROM `gatecsdbms`;";
 ...
 ?>
  ...
  <?php
$sql = "SELECT id,ans FROM gatecscompiler";
...
<h3>The Answers are</h3>
  <?php
$sql = "SELECT id,ans FROM gatecsdbms";
$result = $con->query($sql);
if ($result->num_rows> 0) 
{
echo "<table border=2 width=100% height=50%><tr><th>Q.NO</th><th>Correct Answers</th></tr>";
