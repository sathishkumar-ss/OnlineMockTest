...
<?php                               
                                ...
                                $fetchqry2 = "UPDATE `upsccommunication` SET `userans`='$userselected' WHERE `id`=$c-1"; 
...
<table>
  <?php if(isset($c)) {   $fetchqry = "SELECT * FROM `upsccommunication` where id='$c'"; 
...
<tr><td><h1><br><?php echo @$row['que'];?></h1></td></tr> <?php if($_SESSION['clicks'] > 0 && $_SESSION['clicks'] < 15){ ?>
 ...
 <?php if($_SESSION['clicks']>14){ 
  $qry3 = "SELECT `ans`, `userans` FROM `upsccommunication`;";
 ...
 <h3>The Answers are</h3>
  <?php
$sql = "SELECT id,ans FROM upsccommunication";
