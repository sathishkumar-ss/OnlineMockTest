...
<?php                               
                                ...
                                $fetchqry2 = "UPDATE `ssc2onewordsusbstitution` SET `userans`='$userselected' WHERE `id`=$c-1"; 
...
<table>
  <?php if(isset($c)) {   $fetchqry = "SELECT * FROM `ssc2onewordsusbstitution` where id='$c'"; 
...
<div class="option">
<tr><td><h1><br><?php echo @$row['que'];?></h1></td></tr> <?php if($_SESSION['clicks'] > 0 && $_SESSION['clicks'] < 17){ ?>
...
<?php }  
 ...
 <?php if($_SESSION['clicks']>16){ 
  $qry3 = "SELECT `ans`, `userans` FROM `ssc2onewordsusbstitution`;";
 ...
 <h3>The Answers are</h3>
  <?php
$sql = "SELECT id,ans FROM ssc2onewordsusbstitution";
