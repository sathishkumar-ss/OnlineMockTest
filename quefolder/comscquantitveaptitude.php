...
<?php                               
                                ...
                                $fetchqry2 = "UPDATE `comscquantitiveaptitude` SET `userans`='$userselected' WHERE `id`=$c-1"; 
...
<table>
  <?php if(isset($c)) {   $fetchqry = "SELECT * FROM `comscquantitiveaptitude` where id='$c'"; 
...
<div class="option">
<tr><td><h1><br><?php echo @$row['que'];?></h1></td></tr> <?php if($_SESSION['clicks'] > 0 && $_SESSION['clicks'] < 7){ ?>
...
<?php }  
 ...
 <?php if($_SESSION['clicks']>6){ 
  $qry3 = "SELECT `ans`, `userans` FROM `comscquantitiveaptitude`;";
...
<h3>The Answers are</h3>
  <?php
$sql = "SELECT id,ans FROM comscquantitiveaptitude";
