...
<?php                               
                                ...
                                $fetchqry2 = "UPDATE `defencegrammar` SET `userans`='$userselected' WHERE `id`=$c-1"; 
...
<table>
  <?php if(isset($c)) {   $fetchqry = "SELECT * FROM `defencegrammar` where id='$c'"; 
...
<tr><td><h1><br><?php echo @$row['que'];?></h1></td></tr> <?php if($_SESSION['clicks'] > 0 && $_SESSION['clicks'] < 15){ ?>
 ...
 <?php if($_SESSION['clicks']>14){ 
  $qry3 = "SELECT `ans`, `userans` FROM `defencegrammar`;";
 ...
 ?>
  ...
  <?php
$sql = "SELECT id,ans FROM defencegrammar";
