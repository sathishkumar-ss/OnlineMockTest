...
<?php                               
                                ...
                                $fetchqry2 = "UPDATE `upscic` SET `userans`='$userselected' WHERE `id`=$c-1"; 
...
<table>
  <?php if(isset($c)) {   $fetchqry = "SELECT * FROM `upscic` where id='$c'"; 
...
<div class="option">
<tr><td><h1><br><?php echo @$row['que'];?></h1></td></tr> <?php if($_SESSION['clicks'] > 0 && $_SESSION['clicks'] < 8){ ?>
  ...
  <tr><td><button class="button3" name="click1" onclick="radioValidation()" >View Answer</button></td></tr><?php }  
                ?> 
</div>
 ...
 <?php if($_SESSION['clicks']>7){ 
  $qry3 = "SELECT `ans`, `userans` FROM `upscic`;";
